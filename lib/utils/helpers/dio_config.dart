...
import 'package:gdhote/models/response/response_list_model.dart';
...
class DioListResponseInterceptor implements Interceptor {
  @override
  Future<RequestOptions> onRequest(RequestOptions options) async {
    return options;
  }
  ...
  Future<Response> onResponse(Response response) async {
    if (response.statusCode == HttpStatus.ok) {
      ...
      ResponseListModel responseModel = ResponseListModel.fromJson(result);
